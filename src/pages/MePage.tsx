...
export function MePage() {
  ...
  const refineImportCategories = async (drafts: BillImportDraft[]) => {
    ...
    try {
      ...
      return drafts.map((draft) => {
        ...
        return category && categoryOptions(draft.type).includes(category) ? { ...draft, category } : draft
      })
    } catch {
      return drafts
    }
  ...
  const onImportFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const refined = await refineImportCategories(drafts)
